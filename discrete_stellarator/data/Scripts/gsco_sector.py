...
with modular coils as shown in Section 4.3 of the paper. The wireframe is
initialized to have 3 planar coils per half-period. The toroidal segments
that cross these coils are constrained to carry no current; hence, the 
GSCO procedure is constrained to form coils in between the planar TF coils.
...
a rendering of the wireframe with the constrained segments highlighted, and
the optimzed solution wireframe may also be rendered (requires the mayavi 
package).
"""
...
filename_equil = '../Supporting_Files/input.LandremanPaul2021_QA'

# File specifying the geometry of the wireframe surface (made with BNORM)
filename_wf_surf = '../Supporting_Files/nescin.LandremanPaul2021_QA'
...
pl.savefig(OUT_DIR + 'gsco_sector_constraints.png')
pl.close(pl.gcf())
...
if make_mayavi_plots:
    ...
    mlab.savefig(OUT_DIR + 'gsco_sector_init_plot3d.png')
...
pl.savefig(OUT_DIR + 'gsco_sector_curr2d.png')
pl.close(pl.gcf())
wf.to_vtk(OUT_DIR + 'gsco_sector_model')
...
if make_mayavi_plots:
    ...
    mlab.savefig(OUT_DIR + 'gsco_sector_plot3d.png')
