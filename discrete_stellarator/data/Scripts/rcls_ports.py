...
Optimizes a 12x22 wireframe with the RCLS method as shown in Section 3.3
of the paper. The segment currents of a toroidal wireframe are optimized
in such a way as to leave room for a set of user-specified ports that may
be necessary for diagnostic or heating systems.

Outputs 2d plot of the current distribution in one half-period, as well as
VTK models of the wireframe and the ports. If desired, a 3d plot of the 
wireframe may also be rendered (requires the mayavi package).
"""
...
from helper_functions import equalize_arc_length
...
pl.savefig(OUT_DIR + 'rcls_ports_curr2d.png')
wf.to_vtk(OUT_DIR + 'rcls_ports_model')
...
if make_mayavi_plot:
    ...
    mlab.savefig(OUT_DIR + 'rcls_ports_plot3d.png')
